...
function consolidateWithExistingTranslations(
  filePath,
  fileName,
  language,
  packageDir
  ...
  if (packageDir) {
    ...
    existingTranslationsPath = `./../locales/${language}/${fileName}.json`;
  }
  ...
  if (packageDir) {
    ...
    try {
      if (fs.existsSync(i18nFile)) {
        fs.mkdirSync(
          path.join(__dirname, `./../packages/${packageDir}/locales/tmp`),
          {
            recursive: true,
          }
        );
        ...
        tmpFile = path.join(
          __dirname,
          `./../packages/${packageDir}/locales/tmp/${fileName}.json`
        );

        removeValues(i18nFile, tmpFile);
        consolidateWithExistingTranslations(
          tmpFile,
          fileName,
          language,
          packageDir
        ...
        i18nextToPo(language, fs.readFileSync(tmpFile), {
          ...
          .then(
            save(
              path.join(
                __dirname,
                `./../po-files/${language}/public__${path.basename(
                  fileName
                )}.po`
              ),
              language
